...
impl AppleCheck for Snack{
    fn is_apple(&self) -> bool{
        let mut _return:bool = false;
        match self {
            Self::Apple => {_return = true},
            Self::Cookies(amount) =>{},
...
impl Pricing for Snack{
    fn price(self) -> u8{
        let mut amount: u8 = 0;
        match self {
            Self::Apple => {amount += 5;},
            Self::Cookies(AoC) => {amount += (AoC*2)},
            Self::Sandwich { lettuce, cheese } => {
                amount += 10;
                if lettuce {
                    amount += 1;
...
fn main() {
    ...
    if maybe_fruit.is_some(){
        println!("this is {}", maybe_fruit.unwrap());
    }

    if let Some(x) = maybe_fruit{
        println!("this is {}", x);
    }
    ...
    for number in numbers{
        let res = do_math(number);
        match res{
            Ok(f) => {
                println!("The result was {}", f);
            },
            Err(e) => {
                println!("Error: {}", e);
            }
